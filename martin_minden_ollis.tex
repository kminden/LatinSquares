...
so $\b_\lambda'$ itself is smaller than $\kappa$. %since $\kappa$ is regular.~\footnote{This seems to be the only place where regularity is used, and I don't think it is needed, but singular (non-regular) cardinals are weird.}
...
We enumerate the listed dense sets into a sequence $\set{D_\alpha}{\alpha<\kappa}$ and build a descending chain of conditions $\seq{l_\alpha}{\alpha<\kappa}$ by transfinite induction, ensuring that $l_\alpha\in D_\alpha$ at each step and that $|l_\alpha|<|\alpha+\omega|$ so that the sequence isn't growing at too fast a rate. Note that if a condition $l$ has infinite size $\theta$, the immunization process will add at most $|\theta\cdot 9|$ many new entries to a condition, which has size $\theta$. So the growth constraint on the sequence is reasonable given how we chose to meet dense sets in the above.
